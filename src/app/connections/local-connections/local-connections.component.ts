...
@Component({
  selector: 'app-local-connections',
  templateUrl: './local-connections.component.html',
  styleUrls: ['./local-connections.component.scss']
})
export class LocalConnectionsComponent implements OnInit {
  ...
  getPage(page: number) {
    if (this.activeTab === RightBarCreatorsComponent.GAINERS.name) {
      ...
      return this.altumbaseService
        .getDeSoLockedPage(page + 1, LocalConnectionsComponent.PAGE_SIZE, true)
        .toPromise()
        .then(
          (res) => {
            if (res.length < LocalConnectionsComponent.PAGE_SIZE) {
    ...
    if (this.activeTab === RightBarCreatorsComponent.DIAMONDS.name) {
      this.loadingNextPage = page !== 0;
      return this.altumbaseService
        .getDiamondsReceivedPage(page + 1, LocalConnectionsComponent.PAGE_SIZE, false)
        .toPromise()
        .then(
          (res) => {
            if (res.length < LocalConnectionsComponent.PAGE_SIZE) {
    ...
    if (this.activeTab === RightBarCreatorsComponent.COMMUNITY.name) {
      const start = LocalConnectionsComponent.PAGE_SIZE * page;
      let end = start + LocalConnectionsComponent.PAGE_SIZE;
      if (end > this.globalVars.allCommunityProjectsLeaderboard.length) {
        end = this.globalVars.allCommunityProjectsLeaderboard.length;
      }
      return this.globalVars.allCommunityProjectsLeaderboard.slice(LocalConnectionsComponent.PAGE_SIZE * page, end);
    }
  }

  infiniteScroller: InfiniteScroller = new InfiniteScroller(
    LocalConnectionsComponent.PAGE_SIZE,
    this.getPage.bind(this),
    LocalConnectionsComponent.WINDOW_VIEWPORT,
    LocalConnectionsComponent.BUFFER_SIZE,
    LocalConnectionsComponent.PADDING
  );
